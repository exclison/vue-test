...
route.get("/get-trip-info", async (ctx) => {
  ...
  const { id } = info;
  const hotelSql = `SELECT hotel_id,hotel_name,hotel_phone,room_id,room_num,m.user_id,user.id,user.name AS user_name  FROM user LEFT JOIN (SELECT hotel.hotel_id,hotel.hotel_phone,hotel.hotel_name,room.room_id,room.room_num,room.user_id FROM hotel RIGHT JOIN room ON hotel.hotel_id=room.hotel_id) m ON user.id=m.user_id WHERE id=${id}`;
