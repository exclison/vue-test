 ...
 * @LastEditTime: 2024-02-26 13:14:35
...
export function getParamsForUrl(url: any) {
  ...
  const params = uri ? uri.split("&").reduce((r: any, c: any) => {
...
export class CurrentStep {
  ...
  prev() {
    this.current =
      this.current - 1 >= 0 ? this.current - 1 : this.loop ? this.total - 1 : 0;
  }
